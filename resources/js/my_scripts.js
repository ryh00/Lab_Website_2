...
function loadStatsPage(){
	...
	for(rowC = 2; rowC < table.rows.length; rowC++){
		if(parseInt(table.rows[rowC].cells[2].innerHTML) > parseInt(table.rows[rowC].cells[3].innerHTML))
		{
		...
		loadPlayersPage method:
			...
			purpose: This method will populate the dropdown menu to allow the
					 ...
					 and do the following for each player:
						...
						4. Set the anvarchor tag's text to the player's name.
...
function loadPlayersPage(){
	...
	for(count=0;count<players.length;count++){
		...
		opt.setAttribute('onclick','switchPlayers('+count+')');
		opt.innerHTML = players[count].name;
